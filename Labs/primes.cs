...
// Command-line arguments: run --no-verify d:\UU\PT-Dafny_Labs\Labs\primes.dfy
// primes.dfy
...
method isPrime(x: int) returns (res: bool)
  requires x >= 2
  ensures (res == true && forall j: int {:trigger x % j} :: 2 <= j < x ==> x % j != 0) || (res == false && exists j: int {:trigger x % j} :: 2 <= j < x && x % j == 0)
  decreases x
{
  if x == 2 {
    return true;
    ...
    while i < x
      invariant 2 <= i <= x
      invariant forall j: int {:trigger x % j} :: 2 <= j < i ==> x % j != 0
      decreases x - i
    {
      if x % i == 0 {
        return false;
...
method nextPrime(x: int) returns (r: int)
  requires x >= 2
  ensures x < r
  ensures forall j: int {:trigger r % j} :: x < j < r ==> r % j != 0
  decreases x
{
  var next := x + 1;
...
namespace _module {
  ...
  public partial class __default {
    ...
    {
      BigInteger _0_i;
      _0_i = new BigInteger(25);
      BigInteger _1_prime;
      BigInteger _out0;
      _out0 = __default.nextPrime(_0_i);
      _1_prime = _out0;
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("Next prime number after ")).ToVerbatimString(false));
      Dafny.Helpers.Print((_0_i));
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString(" is ")).ToVerbatimString(false));
      Dafny.Helpers.Print((_1_prime));
    ...
    public static bool isPrime(BigInteger x)
    {
      ...
      if ((x) == (new BigInteger(2))) {
        ...
        while ((_0_i) < (x)) {
          if ((Dafny.Helpers.EuclideanModulus(x, _0_i)).Sign == 0) {
            res = false;
            return res;
          }
    ...
    public static BigInteger nextPrime(BigInteger x)
    {
      ...
      BigInteger _0_next;
      _0_next = (x) + (BigInteger.One);
      bool _1_check;
      bool _out0;
      _out0 = __default.isPrime(_0_next);
      _1_check = _out0;
      while (!(_1_check)) {
        _0_next = (_0_next) + (BigInteger.One);
        bool _out1;
        _out1 = __default.isPrime(_0_next);
        _1_check = _out1;
      }
      r = _0_next;
      return r;
